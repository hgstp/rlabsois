...
To access the press release for the poll, conducted by WIN-Gallup International, use this [link](https://sidmennt.is/wp-content/uploads/Gallup-International-um-tr%C3%BA-og-tr%C3%BAleysi-2012.pdf).
...
# in the data set each row corresponds to the response from one person from one country
```
...
Use the function `dplyr::filter()` to create a new dataframe called `us12` that contains
    only the rows in `atheism` associated with respondents to the 2012 (`"2012"`) survey 
    from the United States (`"United States"`). Next, calculate the proportion of atheist 
...
```{r us-atheism-solution}
library(tidyverse)
us12 <- filter(atheism, nationality == "United States", year == "2012")
summarise(us12, prop = sum(response == "atheist") / n())

# the difference compared to the table is probably just due to rounding
```

## Inference on proportions

Table 6 provides *statistics*, that is, calculations made from the sample of 51,927 people. What we'd like, though, is insight into the population *parameters*. 

You answer the question, "What proportion of people in your sample reported being atheists?" with a statistic; while the question "What proportion of people on earth would report 
...
those used for means in the last chapter: the confidence interval and the  hypothesis test.


If the conditions for inference are reasonable, we can either calculate
the standard error and construct the interval by hand

```{r us-atheism-ci-hand, tidy=FALSE}
...
or write a function `prop_ci()`

```{r prop-ci, tidy = FALSE}
prop_ci <- function(data, success, conf_level = 0.95){
  x <- sum(data == success) 
  n <- length(data)
  p_hat <- x / n
...
```{r prop-ci-us12}
prop_ci(us12$response, success = "atheist")
```
...
for the US data in 2012.

#### Exercise

Using the `prop_ci` function, calculate confidence intervals for the 
    ...
    the `prop_ci` function to construct the confidence intervals.



```{r two-countries-atheism, exercise = TRUE}

```


```{r two-countries-atheism-solution}
# let's consider germany as an example
ger12 <- filter(atheism, nationality == "Germany", year == "2012")

prop_ci(ger12$response, success = "atheist")
...
To do so, we create a `tibble`, which contains a vector `p`, that is a sequence from 0 to 1 with 
each number separated by 0.01, and the margin oferror (`me`) associated with each of these values of `p` using the familiar 
approximate formula ($ME = 2 \times SE$). Lastly, we plot the two vectors 
against each other to reveal their relationship.

```{r me-plot, tidy=FALSE}
n <- 1000
...
```{r sim-np, warning=FALSE, message=FALSE}
set.seed(2019)
p <- 0.1
n <- 1040
...
These commands build up the sampling distribution of $\widehat{p}$ using the 
...
Describe the sampling distribution of sample proportions at $n = 1040$ and 
    $p = 0.1$. Be sure to note the center, spread, and shape.    




```{r sample-dist-prop-setup}
set.seed(2019)
p <- 0.1
n <- 1040
df <- data.frame(p_hats = vector("numeric", 5000))
...
```{r sample-dist-prop, exercise = TRUE}

```


```{r sample-dist-prop-solution}
summary(df$p_hats)
...
# the sampling distribution also follows nicely the 68-95-99.7 rule
...
```{r sample-dist-prop-4, exercise = TRUE}

```


```{r sample-dist-prop-4-solution, warning=FALSE, message=FALSE}
set.seed(2019)
...
-   Answer the following two questions using the `prop_.test_ci()` function. As 
