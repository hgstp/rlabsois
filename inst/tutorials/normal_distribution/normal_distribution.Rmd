...
useful to group the data based on `sex`.

```{r male-female, message=FALSE, warning=FALSE}
library(tidyverse)
bdims <- bdims %>%
  group_by(sex)
```
...
#### Exercise 
Make a histogram of men's heights and a histogram of women's heights.  Try to use
`facet_wrap()` to create side by side histograms.



```{r male-female-hist, exercise = TRUE}

```


```{r male-female-hist-solution}
ggplot(bdims, aes(x = hgt)) + geom_histogram(bins=15) + facet_wrap(~ sex)
```


## The normal distribution

The shape of the distributions can be described as *bell-shaped* 
...
Next we make a density histogram to use as the backdrop and use `stat_function()` 
to overlay a normal density. The difference between a 
...
In `geom_histogram()` we need to specify the `y` aesthetic to be the computed variable `density`. Since the variable `density` is computed and not contained in the data set `bdims`, this must be marked by the special notation `..density..`.
With `stat_function()` one can draw any kind of function on an existing plot. In this case we plot the density curve `dnorm` of a normal distribution with mean `r hgt_mean_sd$mean` and standard deviation `r hgt_mean_sd$sd`.



#### Exercise
Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
...
specify to be the same number of heights as we have observations from women in the data set. This is done by computing the number of observations, which satisfy `sex == 0`.

The last two arguments determine the mean and standard  deviation of the normal distribution from which the simulated sample will be 
generated.  We can take a look at the shape of our simulated data set, `sim_norm`, 
as well as its normal probability plot.


#### Exercise
Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
...
```{r sim-norm-qq, exercise = TRUE}

```


```{r sim-norm-qq-solution}
...
#### Exercise
Does the normal probability plot for female observations look similar to the plots 
...
adult female is taller than 6 feet (about 182 cm)?" (*The study that published this data set is clear to point out that the sample was not random and therefore inference to a general population is not suggested.  We do so here only as an exercise.*)

If we assume that female heights are normally distributed (a very close 
approximation is also okay), we can find this probability with the function `pnorm`.

```{r pnorm}
1 - pnorm(q = 182, mean = hgt_mean_sd$mean, sd = hgt_mean_sd$sd)
```
...
take one minus that probability. An equivalent way to compute is to specify `lower.tail = FALSE`

```{r pnorm-upper-tail}
pnorm(q = 182, mean = hgt_mean_sd$mean, sd = hgt_mean_sd$sd, lower.tail = FALSE)
```
...
```{r height-weight, exercise = TRUE, exercise.eval = TRUE}
females <- bdims %>%
  filter(sex == 0)
```
...
-   As you can see, normal probability plots can be used both to assess 
    normality and visualise skewness.  Make a normal probability plot for female 
...
p1 <- ggplot(females, aes(x = scale(bii.di))) + geom_histogram(bins=15) +
  xlab("standardised bii.di values")
p2 <- ggplot(females, aes(x = scale(elb.di))) + geom_histogram(bins=15) +
  xlab("standardised elb.di values")
p3 <- ggplot(females, aes(x = scale(age))) + geom_histogram(bins=15) +
  xlab("standardised age values")
p4 <- ggplot(females, aes(x = scale(che.de))) + geom_histogram(bins=15) +
  xlab("standardised che.de values")

gridExtra::grid.arrange(p1,p2,p3,p4, ncol = 2)

p3 <- ggplot(females, aes(sample = scale(bii.di))) + 
  stat_qq() + stat_qq_line() +
  ggtitle("normal probability plot C")
p1 <- ggplot(females, aes(sample = scale(elb.di))) +   stat_qq() + stat_qq_line() +
  ggtitle("normal probability plot A")

p2 <- ggplot(females, aes(sample = scale(age))) +   stat_qq() + stat_qq_line() +
  ggtitle("normal probability plot B")

p4 <- ggplot(females, aes(sample = scale(che.de))) +   stat_qq() + stat_qq_line() +
  ggtitle("normal probability plot D")
